...
(function ($) {
	...
	$.fn.FancyFileUpload = function (options) {
		...
		var AddFile = function (uploads, e, data) {
			...
			// add files to internal file list
			storedFiles.push(data.files[0].name);
			...
			var RemoveFile = function (e) {
				...
				if (inforow.hasClass('ff_fileupload_uploading')) {
					...
					storedFiles = storedFiles.filter(function (e) { return e !== fileName });
				}
				else {
					...
					if (settings.removed) settings.removed.call(data.ff_info.inforow, e, data);
			...
			if (settings.maxfilecount > -1 && storedFiles.length > settings.maxfilecount) data.ff_info.errors.push(FormatStr(Translate('Maximum number of files exceeded.  Maximum number of files is {0}.'), settings.maxfilecount));
